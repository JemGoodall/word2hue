...
def embeddings_to_rgb(text_input, n_tokens=-1, keep_punctuation=True, reduction_method='pca'):
    tokens = get_tokens(text_input, keep_punctuation, n_tokens)
    word_embeddings = np.array([model(token).vector for token in tqdm(tokens,'Getting word vectors')])
